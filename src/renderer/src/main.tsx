...
if (!window.responsiver) {
  window.responsiver = {
    ...
    runMatrix: async () => { throw new Error('La matrice reproductible est disponible uniquement dans Electron.') },
    onMatrixProgress: () => () => undefined,
    verifyStaging: async () => { throw new Error('Correction Express est disponible uniquement dans Electron.') },
    applyVerifiedStaging: async () => { throw new Error('L’application vérifiée est disponible uniquement dans Electron.') },
