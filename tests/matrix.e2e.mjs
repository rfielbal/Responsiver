...
try {
  ...
  assert.match(await page.locator('.matrix-verdict').textContent(), /Comparaison validée/)
  assert.ok(await page.locator('.matrix-cell').count() >= 6)
  await page.locator('.matrix-page').screenshot({ path: join(screenshotRoot, 'matrix-responsive.png'), animations: 'disabled' })
