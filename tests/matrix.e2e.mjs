...
try {
  ...
  const summarizedCell = page.locator('.matrix-cell').filter({ hasText: /priorité/ }).first()
  await summarizedCell.waitFor({ state: 'visible' })
  assert.match(await summarizedCell.textContent(), /observation/, 'Une cellule doit distinguer les priorités regroupées des observations répétées.')
  assert.doesNotMatch(await board.locator('header').textContent(), /ouvrir dans le Laboratoire/i, 'La grille ne doit plus annoncer une navigation immédiate et ambiguë.')
  ...
  const cellDetail = page.locator('.matrix-cell-detail')
  await cellDetail.waitFor({ state: 'visible' })
  await board.waitFor({ state: 'visible' })
  assert.match(await cellDetail.textContent(), /Lecture de la cellule.*Mobile.*Navigation ouverte/s)
  assert.match(await cellDetail.textContent(), /regroup|aucune anomalie|assez de données/i, 'Le détail doit expliquer le sens du résultat avant de quitter la Matrice.')
  assert.match(await cellDetail.locator('.matrix-cell-detail__footer').textContent(), /index\.html.*393 × 852.*Navigation ouverte/s, 'Le contexte qui sera restauré doit être annoncé explicitement.')
  await page.screenshot({ path: join(screenshotRoot, 'matrix-cell-detail.png'), animations: 'disabled' })
  await cellDetail.getByRole('button', { name: 'Ouvrir cette vue dans le Laboratoire' }).click()
  await page.locator('.stage-canvas iframe').first().waitFor({ state: 'visible' })
