...
test('le runner sert uniquement localhost, injecte le bridge et gère les médias partiels', async (context) => {
  ...
  assert.match(pageBody, /inset-right/)
  assert.match(pageBody, /--handle-right/)
  assert.match(pageBody, /inheritedProperties/)
  assert.match(pageBody, /handle\.addEventListener\('pointerup', commitDesignGesture/)
  assert.match(pageBody, /designHost\.addEventListener\('pointerup', commitDesignGesture/)
  ...
  assert.match(pageBody, /after: 'min\(' \+ nextWidth \+ 'px, calc\(100vw - 8px\)\)'/)
  assert.match(pageBody, /after: replaced \? 'auto' : nextHeight \+ 'px'/)
