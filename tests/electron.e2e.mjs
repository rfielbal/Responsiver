...
try {
  ...
  const navigationRow = page.locator('.issue-row').filter({ hasText: 'Navigation déséquilibrée à cette largeur' })
  ...
  assert.match(await page.locator('.change-plan-bar').textContent(), /1 en attente/)
  await page.locator('.change-plan-bar').getByRole('button', { name: 'Examiner' }).click()
  ...
  }, '/journal.html').catch(async (error) => {
  ...
  await beforeAfter.waitFor({ state: 'visible' })
  await afterDocument.locator(revealSelector).waitFor({ state: 'attached' })
  await page.locator('.proposal-decision').getByRole('button', { name: 'Ajouter au plan' }).click()
  ...
  await themeDecision.getByRole('button', { name: 'Ajouter au plan' }).click()
  ...
  await instructionDecision.getByRole('button', { name: 'Ajouter au plan' }).click()
  ...
  const undoButton = page.locator('.change-plan-bar').getByRole('button', { name: 'Annuler' })
