...
test('le pont de scroll distant ne collecte ni texte, ni sélecteur applicatif, ni formulaire', async () => {
  ...
  assert.match(runtime, /const rootScrollable = rootMaxX > 1 \|\| rootMaxY > 1/)
  assert.match(runtime, /const needsContainerScan = hasPreferredContainer \|\| preferredContainer === undefined && !rootScrollable/)
  assert.match(runtime, /includeRuntime \? \{ snapshot, scroller, nodes \} : snapshot/)
