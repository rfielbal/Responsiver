...
La version ajoute un Studio de un à cinq écrans pour projet local, URL publique ou localhost, un catalogue de plus de 60 profils, des suites réutilisables, un pilotage sélectif et trois dispositions. La superposition de maquette et la capture de planche complètent les runners locaux. Elle conserve la Matrice Chromium isolée, le comparateur de signaux stables, le jeton d’application lié au staging exact, le traçage CSSOM, l’Atelier visuel, l’espace Code, l’assistant local et le compagnon Chrome. Le laboratoire distingue exploration interactive et preuve anti-régression.
...
| Organiser la planche | Alignés, Grille ou Focus, rotation par écran | Le viewport CSS reste celui du profil, même si la preview est réduite visuellement |
...
- **Bridge Studio fermé** : navigation, scroll sémantique et interactions explicitement sûres entre les iframes locales ; navigation et scroll structurel via IPC pour les vues distantes.
...
Le mode **Appareil** reste la surface de précision : un viewport, redimensionnement manuel, zoom, inspection, ciblage d’un constat et Avant/Après. Le mode **Studio** est une planche d’exploration disponible pour un projet local, une URL publique ou un localhost. Une suite charge de un à cinq profils ; l’utilisateur peut ensuite changer la disposition, choisir le pilote, isoler un écran, pivoter un modèle compatible et redimensionner sa vue. Sur un runner local, il peut aussi ajouter une maquette et capturer la composition visible.

Le pilote partage la navigation et le défilement avec les vues liées ; une vue isolée conserve son propre parcours. Sur le runner local, le scroll utilise un élément sémantique puis la progression du conteneur dominant en repli, y compris une zone interne `overflow` de dashboard ou de SPA. Les interactions restent désactivées par défaut et leur protocole ne rejoue que des commandes bornées et jugées non sensibles : aucun lien, submit, fichier, mot de passe, coordonnée, libellé destructif ou élément ambigu n’est diffusé.

Pour une URL ou un localhost, Responsiver conserve une vue native principale et crée au besoin jusqu’à quatre `WebContentsView` supplémentaires, chacune avec son stockage éphémère et sa politique réseau. La route du pilote est propagée aux seules vues liées. Le scroll transmet uniquement progression, type de repère structurel et rang, sans texte ni sélecteur. Les clics, saisies et formulaires restent toujours propres à chaque vue. La maquette et la capture groupée sont désactivées, car ces surfaces Chromium natives ne peuvent pas être composées de façon fiable dans l’overlay React.

Cette planche ne remplace pas la page **Matrice**. Le Studio est volontairement interactif et dépend des gestes de la session ; sa capture est une preuve visuelle de travail. La Matrice repart au contraire d’états fermés, nettoie le stockage de chaque cellule et compare les mêmes routes et formats entre source et candidat. Seule cette seconde chaîne peut contribuer au jeton Correction Express.
...
- `npm run test:e2e:studio` : cinq écrans, suites, Focus, isolation, navigation, scroll, interactions sûres, maquette et bibliothèque vérifiés ;
- `npm run test:e2e:remote` : redirection, audit mobile, formulaire localhost, vues natives multiples, navigation et scroll synchronisés vérifiés ;
