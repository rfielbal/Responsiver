export const MAX_COMPANION_URL_LENGTH = 8192
...
export function normalizeNativeHostUrl(value) {
  if (
    typeof value !== 'string' ||
    value.length === 0 ||
    value.length > MAX_COMPANION_URL_LENGTH ||
    /[\u0000-\u001f\u007f]/.test(value)
  ) return null
  ...
  try {
    ...
    return parsed.href.length <= MAX_COMPANION_URL_LENGTH ? parsed.href : null
  } catch {
    return null
  }
}
